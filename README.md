...
As the TEDS data is collected quarterly while military aid statistics are yearly, grouped aid data in the merged aid dataframe needs to be expanded so each year is represented by four rows of identical spending values but different `year` IDs corresponding with the TEDS data increments over that same year. This inevitably results in more of a "step" chart but allows data to be merged on the same key and graphed on the same scale.
