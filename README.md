...
#### Charts and analysis
TBA, have to convert to chartable values / `matplotlib.pyplot`, `int()`
### 4. dippol.py
----
_//DRAFT - WILL PROBABLY REMOVE_
...
- _Taiwan has the same polity score for all years in the dataset (transitioned from martial law prior to 2000) so this is not interesting. Number of diplomatic allies pretty consistently just decreases over time so also uninteresting, not many to begin with. only actual interesting part is distribution of allies polity scores_
- _planned script output would not be chartable against other data so can only function as dictionary of snapshot of distribution per year, probably getting too tangential + also it hurts my brain_
### 5. compgraphs.py
----
//TBA, VERY DRAFT
